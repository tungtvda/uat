    ...
    protected function ForgotPasswordProcess()
    {
        ...
        if($this->section=='member')
        {
            ...
            if ($param['status']['ok']=="1")
            {
                ...
                $mailer->From = $param['config']['CONTACT_EMAIL_TO'];
                $mailer->AddReplyTo($param['config']['CONTACT_EMAIL_TO'], $param['content']['Agent']['Company']);
                $mailer->FromName = $param['content']['Agent']['Company'];

                $mailer->Subject = "[".$param['content']['Agent']['Company']."] New password request";
