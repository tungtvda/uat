<div class="bg-header">
	...
	<?php } else { ?>
        ...
        <div id="member_in_box"><a href="/agent/agent/dashboard">Dashboard</a>

            <?php #if($_SESSION['agent']['TypeID']=='1'){ ?>
            <a href="<?php echo $data['config']['SITE_DIR']; ?>/agent/member/group?page=all">Group Members</a>
            <?php #} ?>

            <a href="<?php echo $data['config']['SITE_DIR']; ?>/agent/transaction/group?page=all">Group Transactions</a>

            <?php if (($_SESSION['agent']['TypeID']=='1')||($_SESSION['agent']['operator']['OperatorType']=='1')) { ?>
            <a href="<?php echo $data['config']['SITE_DIR']; ?>/agent/wallet/index?page=all">Wallets</a>
            <?php } ?>
            ...
            <?php } ?>
                ...
                <?php #if($_SESSION['agent']['TypeID']=='1'){ ?>
                <a class="personal" href="<?php echo $data['config']['SITE_DIR']; ?>/agent/member/index?page=all">Personal Members</a>
                <a class="personal" href="<?php echo $data['config']['SITE_DIR']; ?>/agent/transaction/index?page=all">Personal Transactions</a>
                <?php #} ?>
            ...
            <?php } else if ($_SESSION['agent']['operator']['ProfileName']=='Analyst') { ?>
            <div class="welcome">Welcome, <span id="nameBold"><?php if(isset($_SESSION['agent']['Name'])===TRUE && empty($_SESSION['agent']['Name'])===FALSE){ echo $_SESSION['agent']['Name'];?></span>.&nbsp;&nbsp;&nbsp;My Real Balance (MYR): <?php Core::getHook('Credit-Balance'); ?><?php }elseif(isset($_SESSION['agent']['operator']['ProfileID'])===TRUE && empty($_SESSION['agent']['operator']['ProfileID'])===FALSE){ echo $_SESSION['agent']['operator']['Username']; ?>&nbsp;&nbsp;&nbsp;<?php } ?></div>
            <div id="member_in_box">
                <a href="/agent/agent/dashboard">Dashboard</a>
                <a href="<?php echo $data['config']['SITE_DIR']; ?>/agent/transaction/group?page=all">Group Transactions</a>
                <a href="<?php echo $data['config']['SITE_DIR']; ?>/agent/agent/agentlist">Agent List</a>
                <a href="<?php echo $data['config']['SITE_DIR']; ?>/agent/transaction/affiliatedreporting?page=all">Affiliate Report</a>
                <a href="<?php echo $data['config']['SITE_DIR']; ?>/agent/agent/report?page=all">Monthly Report</a>
                <a class="personal" href="<?php echo $data['config']['SITE_DIR']; ?>/agent/transaction/index?page=all">Personal Transactions</a>
                <a class="personal" href="<?php echo $data['config']['SITE_DIR']; ?>/agent/agent/profileview">Profile</a>
                <a class="personal" href="/agent/agent/logout">Log Out</a>
            </div>
